...
class BackUsersController extends FrontBaseController {
	...
	{
		$entries = User::orderBy('type', 'desc')->orderBy('created_at', 'desc')->paginate(10);

		return View::make('backend.' . $this->view . '.index')
			->withEntries($entries);
	}

	public function create()
	{
		return View::make('backend.' . $this->view . '.create');
	}
	...
	public function store()
	{
		...
		else
		{
			...
			$entry->username 	= Input::get('username');

			if (Input::get('password'))
				$entry->password 	= Hash::make((string) Input::get('password'));
			...
			return Redirect::route('users.index')->withCreated($entry->id);
		}
	}
	...
	public function edit($id)
	{
		...
		if ($entry)
		{
			return View::make('backend.' . $this->view . '.edit')
				->withEntry($entry);
		}
	...
	public function update($id)
	{
		...
		if ($entry)
		{
			...
			else
			{
				...
				return Redirect::route('users.index')->withUpdated($entry->id);
			}
		}
	...
	public function destroy($id)
	{
		...
		if ($entry)
		{
			...
			return Redirect::route('users.index')->withDeleted($entry->id);
		}
